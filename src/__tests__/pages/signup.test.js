...
describe('<SignUp />', () => {
  it('renders the sign up page with a form submission', async () => {
    ...
    await act(async () => {
      ...
      await fireEvent.change(getByPlaceholderText('Email address'), { target: { value: 'kingo@gmail.com' } });
      await fireEvent.change(getByPlaceholderText('Password'), { target: { value: 'password' } });
      fireEvent.click(getByTestId('sign-up'));

      expect(getByPlaceholderText('Email address').value).toBe('kingo@gmail.com');
